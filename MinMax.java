...
class MinMax{
  ...
  MinMax(Board tabuleiro){
    this.tabuleiro = tabuleiro;
    nextMoveColumn = -1;
    valueMinMax(tabuleiro,true);
  }
  ...
  int valueMinMax(Board tabuleiro, boolean isMax){
      ...
      for(int j = 6; j >= 0; j--){
        for(int i = 5; i >= 0; i--){
          ...
          if(tabuleiro.matrix[i][j] == ' '){
            ...
            best2 = Math.max(best,valueMinMax(newtabuleiro,!isMax));
            if(best != best2){
              newtabuleiro.custo = best2;
              best = best2;
              if(newtabuleiro.depth == 1)
      ...
      for(int j = 6; j >= 0; j--){
        for(int i = 5; i >= 0; i--){
          ...
          if(tabuleiro.matrix[i][j] == ' '){
            ...
            best = Math.min(best,valueMinMax(newtabuleiro,!isMax));
            break;
          }
        }
      }
